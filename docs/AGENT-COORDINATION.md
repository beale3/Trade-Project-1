...
| Backend-Data = **SDE1** | ▶ **LIVE** (`Trade - SDE1`, self-verified against the repo 2026-08-31, Lead-confirmed) | 🔴 **FORMALLY DISPATCHED 2026-08-31 (D-TRADE-037) — Gate 1 of 2** | **Task:** build `helm/ingest`, pull real historical price/volume data, produce one real CLEARED/DROPPED verdict on a real component — replacing the synthetic-only coverage the build chain used through Stage 4. **Do NOT execute any data pull yet.** Gate 1 (now): propose a ticker universe + historical window and bring it back to the Lead for Director approval. Gate 2 (separate, later, not yet authorized): only after the Director approves that scope does the actual pull execute. Same chain discipline as Stages 1-4: AIQ independently re-derives/audits your result (not trusting your report), QA re-runs for reproducibility, Lead verifies everything at source. **Known open risk to factor into scope-sizing:** `helm/spend/` (the NN-8 spend-cap guard) doesn't exist yet, so keep the proposed pull small/bounded — this will be raised to the Director before Gate 2 regardless. |
