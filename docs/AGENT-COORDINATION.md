...
and Lead-absorbed into canonical as D-TRADE-030 — the design work is DONE.** 🟡 D-TRADE-010 (no-build)
is **still not re-ruled** — see the flagged recommendation in `stage-plan.md`; this is the single
remaining blocker on an actual first CV run alongside P-3 (universe, routed to Director) and P-4
(OP-1/2/3 pre-registration values, routed to Director). Every seat below was messaged directly with its
scope at each pivot; **design work proceeds; broad build waits on Director confirmation of D-TRADE-010's
scope**, same as any other wave-entry gate.
...
| **DevOps** | ▶ **LIVE** (`Trade - DevOps`) | ▶ live · **⏸ holding (HELM) / ▶ active (D-TRADE-023)** | **HELM Phase-1:** gate-harness DESIGN done + co-signed ADR-0001's NN-7/NN-8/Lane-E non-negotiables (`docs/roles/devops/harness-design.md`); **holding on scaffold files** — ADR-0001's own P-1 confirms no build until the Director re-scopes D-TRADE-010. **D-TRADE-023 (equity dashboard, separate initiative, no freeze):** ADR-0002's 3 DevOps deliverables **DELIVERED** — `flask==3.1.3` installed, `.claude/launch.json` run entry, `scripts/smoke_rolling_watchlist_web.py` boot check (SKIPs cleanly pending AI/ML's `app.py`), `tools/web/README.md` (run steps + server-only Massive-key contract). Ready for AI/ML/Designer to wire against. |
| **AI/ML** | ▶ **LIVE** (`Trade - AI-ML`) | ▶ live · ⏸ holding on HELM build (P-1); actively delivering elsewhere | **HELM Phase-1:** co-signed ADR-0001 R2 in full (Leg A + trailing-stop mechanics + Leg B/NN-10), grounded the Architect's design with real `tools/rolling_watchlist.py` line citations. No IV-rank/DTE — dropped with options. Ready to build the CV pipeline + the trailing-stop mode the moment P-1 clears; nothing else blocking. **Also DELIVERED (D-TRADE-023/031, not build-frozen):** the dashboard backend (`tools/web/{app.py,scan_service.py,serialize.py}`), a real end-to-end browser integration test (found+fixed a live bug), and D-TRADE-031 (`min_float` guardrail param + float-distribution script, correctly caveated against the float study's NO-GO). |
