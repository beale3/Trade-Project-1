...
| **AI/ML** | ▶ **LIVE** (`Trade - AI-ML`, session `local_fadecaf6`) | ✅ **STAGE 1 DELIVERED 2026-08-31 (`006db52`/`f8f685e`), Lead-verified at source, not just relayed** | Self-verified D-TRADE-034/035/036 before starting (per the Director's requirement), independently rediscovered + safely stashed the unexplained WIP (item 18) rather than build on it. **Delivered:** trailing-stop mode in `simulate_day_trades()` (§6.3 formula exactly, backward-compatible, bar-causal), `helm/screener/adapter.py` (OP-4's 9-component list), `helm/validation/engine/{bar,harness,leg_a,leg_b}.py` (D-TRADE-021/029 bar, D-TRADE-036's locked values). **Lead independently re-ran leg_b.py's core logic against synthetic planted-effect/noise/thin-support/sensitivity-cell data — exact match with AI/ML's claims on all four.** Flagged Leg B's methodology adaptation (no fitted model once exit params are fixed constants — paired-comparison reuse of the LOO/5-fold shape) explicitly for AIQ's independent judgment, not baked in silently. Does NOT include `_gates` wiring or a real-data backtest run — stated plainly. **STAGE 1 of 4.** Build the entry-signal (Leg A) + trailing-stop exit-rule (Leg B) logic: the trailing-stop mode in `tools/rolling_watchlist.py` (§6.3 formula, primary clearance-eligible grid cell trail=8%/init=3%, D-TRADE-036) + `helm/screener` (thin adapter) + `helm/validation/engine` (CV harness, D-TRADE-021 bar, NN-10 train-fold-only parameter isolation). `helm/universe` explicitly NOT part of this — dropped (D-TRADE-035). Report to the Lead on completion — **staged reporting requested by the Director, not batched to the end.** Prior deliverables unchanged: ADR-0001 R2 co-sign, D-TRADE-023 dashboard backend, D-TRADE-031. |
| **AI Quality** ("AIQ") | ▶ **LIVE** (`Trade - AIQ`, session `local_7adfedb4`) | 🔴 **STAGE 2 DISPATCHED 2026-08-31 — AI/ML's Stage 1 delivered, audit this now** | Caught its own stale "holding for P-1" text earlier, re-synced correctly (`bbf7a28`, Lead-verified). **Now dispatched against a real deliverable:** independently re-derive Leg A + Leg B from raw `tools/rolling_watchlist.py` primitives — never import `helm/screener`'s or `engine`'s outputs (NN-3, builder≠judge). **AI/ML specifically flagged Leg B's methodology for your independent judgment** (`helm/validation/engine/leg_b.py`'s docstring): no fitted model exists once D-TRADE-036 fixed the exit params as constants, so the LOO/5-fold harness was adapted to a paired-outcome comparison instead of a regression — if that translation is unsound, the method is the defect, not the numbers. Prior: ADR-0001 R2 protocol-17 co-sign delivered (3 real objections found+fixed) — same rigor expected here. |
