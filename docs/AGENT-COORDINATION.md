...
| Backend-Data = **SDE1** | ▶ **LIVE** (`Trade - SDE1`, self-verified against the repo 2026-08-31, Lead-confirmed) | 🔴 **GATE 2 APPROVED 2026-08-31 (D-TRADE-038) — DISPATCHED TO EXECUTE** | Scope cleared as-is: 100 tickers, 2024-06-01→pull-date−45d, ~120 expected events. **Standing condition attached by the Director: rate-limit + `helm/spend/` remain unconfirmed/unarmed, tolerable for this run's small size ONLY — not cleared for any future scaled-up pull.** Build `helm/ingest`, execute the approved pull, produce one real CLEARED/DROPPED verdict. Full chain discipline: AIQ independently re-derives/audits (not trusting the report), QA re-runs for reproducibility, Lead verifies at source throughout. Two report-backs owed: pull complete, and the real verdict produced. |
