...
| Principal Architect | ▶ **LIVE** (`Trade - Architect`, Fable5·Max) | ▶ live · ⏸ holding | **ADR-0001 @ Revision 2 DELIVERED** (D-TRADE-028 — options dropped → equity buy signals + trailing-stop; `docs/adr/ADR-0001…`, PROPOSED @ origin). R2: two-leg label (entry-signal + trailing-stop-vs-fixed-holding), trailing-stop defined as a backward-compat mode in shared `tools/rolling_watchlist.py`, new NN-10 (exit-param isolation), `helm/screener`→thin adapter, `helm/universe` likely drops (P-3); §14 A6a/A6b delta. HOLDING for co-sign (**AIQ load-bearing on label/NN-10**) + Director GO. Build-blockers: P-1 (D-TRADE-010), P-3 (universe), P-4 (label params), P-5 (B5); P-2 MOOT. **Also DELIVERED:** ADR-0002 (D-TRADE-023 web UI, `tools/web/`, 3 build tasks dispatched). Next on GO: A6 ASR after build. |
