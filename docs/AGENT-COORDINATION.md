...
| **AI/ML** | ▶ **LIVE** (`Trade - AI-ML`, session `local_fadecaf6`) | ✅ **STAGE 1 DELIVERED 2026-08-31 (`006db52`/`f8f685e`), Lead-verified at source, not just relayed** | Self-verified D-TRADE-034/035/036 before starting (per the Director's requirement), independently rediscovered + safely stashed the unexplained WIP (item 18) rather than build on it. **Delivered:** trailing-stop mode in `simulate_day_trades()` (§6.3 formula exactly, backward-compatible, bar-causal), `helm/screener/adapter.py` (OP-4's 9-component list), `helm/validation/engine/{bar,harness,leg_a,leg_b}.py` (D-TRADE-021/029 bar, D-TRADE-036's locked values). **Lead independently re-ran leg_b.py's core logic against synthetic planted-effect/noise/thin-support/sensitivity-cell data — exact match with AI/ML's claims on all four.** Flagged Leg B's methodology adaptation (no fitted model once exit params are fixed constants — paired-comparison reuse of the LOO/5-fold shape) explicitly for AIQ's independent judgment, not baked in silently. Does NOT include `_gates` wiring or a real-data backtest run — stated plainly. **STAGE 1 delivered; now owes 4 fixes from AIQ's Stage 2 audit (`cd079eb`, messaged directly per protocol 11), before Stage 3 makes sense:** (1) Leg B's LOO outlier statistic is empirically too weak — needs a real per-estimate sign-disagreement check, not just the mean; (2) Leg B's verdict needs to disclose it's a stability check, not a generalization test (LL-40); (3) `bar.py` must return `DROPPED`, not `NOT_CLEARED`, to match the ratified 4-state schema; (4) `leg_b.py`'s undocumented 5th state (`SENSITIVITY_ONLY_WOULD_CLEAR`) needs to either become a ratified schema extension or be re-expressed within the existing 4 states + a separate boolean field. |
