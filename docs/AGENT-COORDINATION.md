...
| Backend-Data = **SDE1** | ▶ **LIVE** (`Trade - SDE1`, self-verified against the repo 2026-08-31, Lead-confirmed) | ✅ **GATE 1 DELIVERED 2026-08-31 (`3ef2aae`), Lead-verified at source** — holding for Gate 2 | **Proposed:** 100 tickers (subset of the short-interest study's proven 754-ticker/917-event cohort, not a fresh guess) · 2024-06-01 → pull-date−45d (buffer rule, keeps the 1-month Leg-A horizon fully realized) · ~120 expected guardrail events, ~4x margin over the 30-event floor · sizing bound on API call count (~100 calls), not date range, given no spend guard yet. **Lead independently re-checked every number** (event/ticker counts against the real study file, scanner default thresholds against `tools/rolling_watchlist.py`, date range) — exact match, nothing invented. One flagged unknown: Massive's exact rate-limit isn't documented in-repo, recommended DevOps/FinOps confirm before Gate 2. Component choice correctly deferred to post-Gate-2 build. **Will not pull any data until Gate 2 clears.** |
