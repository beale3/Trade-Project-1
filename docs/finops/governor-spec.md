...
|---|---|---|---|
| **GB-downloaded-this-month cap, SEC-API.io specifically** | set well under the **50 GB (Personal tier) / 100 GB (Business tier)** included volume — e.g., a headroom fraction, not the exact boundary | this is the one **measured, real** overage line (`$0.30/GB`, `cost-model.md` §2.3) in the current provider set — worth a real value, not just a placeholder | ▸ needs `<2.1>` confirmation the key IS SEC-API.io + which tier, before a precise number is set |
