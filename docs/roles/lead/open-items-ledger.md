...
| 14 | 🔴 **REOPENED 2026-08-30 — S5 (Massive) command-bar exposure.** Briefly closed, then reopened same day. Director-confirmed rotation + Lead's independent verification (template git history clean, real file gitignored/never committed, current key live HTTP 200) all still stand. SecOps re-confirmed its co-sign on this basis (`1ee2682`) and, per its own recommendation, the Lead checked shell/terminal history for a leaked raw value — PowerShell history, bash history, Windows Run MRU all clean, **but a scan of this project's local Claude Code session transcripts turned up one candidate that does not read as a placeholder**: `517ca982-2b50-41cb-ab85-4da846eb94f2.jsonl`, line 1571, dated 2026-08-23. A SHA-256 hash comparison against the current key (would resolve this without ever displaying either value) was **blocked by the permission classifier**. Value never displayed at any point. | **Needs the Director's decision: grant the specific permission for a hash-only comparison, personally inspect that transcript file directly, or treat it as a precaution and rotate S5 again regardless.** Not resolved — reported rather than guessed past. |
