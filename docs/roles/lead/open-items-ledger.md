...
| 14 | 🔴 **NEW — S5 (Massive) command-bar exposure, reported 2026-08-30, NOT YET independently verified, rotation status UNKNOWN.** Director reported (after initially misattributing this to S6, corrected same session): the Massive API key was pasted into a command bar the first time it was used; the Director's partner flagged it as bad practice at the time. Logged in `b5-secret-approval-checklist.md` Step 1. Predates and was not covered by SecOps's 2026-08-30 Step 2 review — that review's S5 co-sign may need re-confirming once this is resolved. | **Blocking question: was the S5 key ever rotated after that incident?** If not, this is a live unremediated exposure needing the same rotate-and-verify sequence as S6, not just a historical note — asked directly, awaiting the Director's answer. |
